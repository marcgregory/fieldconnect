...
| TD-008 | Hand-rolled forms in ScheduleForm, ClockInOut, JobDetailClient still need migration to react-hook-form + zod (Note: ClockInOut and JobDetailClient verified — no migration needed; ScheduleForm remains to audit) | Medium | Sprint 6 form-architecture work migrated auth forms + ProjectForm only. Original TD claimed "old `useState` + `FormData` + manual `safeParse` pattern" for all three, but audit confirmed ClockInOut already uses RHF, and JobDetailClient has zero `safeParse` calls, single-text-field note input (no RHF benefit), and file uploads correctly using `FormData`. | Form validation drift between client and API; inconsistent user experience across the app. | Sprint 7 (post-Sprint 6) | Developer | Part 2: Done ✅ / Part 3: Not Needed / ScheduleForm: Pending |
| TD-008-P2 | ClockInOut — reviewed and confirmed already using `useForm` + `zodResolver` with `clockInFormSchema` (742 lines). The functional `useState` instances are legitimate UI state (activeEntry, GPS, timer), not form state. TD-008 tracking label was stale. | Medium | ClockInOut already uses RHF + Zod for form fields. No migration needed. | None. | Sprint 7 | Developer | Done ✅ |
| TD-008-P3 | JobDetailClient — full audit completed (1649 lines). Confirmed: zero `safeParse` calls, 2 `FormData` usages (both for file uploads — correct pattern), 1 text input (single useState — no RHF benefit), 19 useState hooks total (all server data, UI state, workflow state, or loading flags). Not a form-migration candidate. | Medium | Full manual code audit verified the component patterns are appropriate for their purpose. No stale form-state claims found. | None. | Sprint 7 | Developer | Closed — Not Needed ✅ |
| TD-009 | No periodic cleanup job for expired rate_limit_events and login_lockouts rows | Low | Phase 4 added inline cleanup on every lockout check, but old rate_limit_events rows for `login-ip:*` scopes accumulate. | `rate_limit_events` table grows unbounded. | Sprint 7 (post-Sprint 6) | Developer | Done ✅ |
