...
| TD-008 | Hand-rolled forms in ScheduleForm, ClockInOut, JobDetailClient still need migration to react-hook-form + zod | Medium | Sprint 6 form-architecture work migrated auth forms + ProjectForm only. Original TD claimed "old `useState` + `FormData` + manual `safeParse` pattern" for all three. Full manual audit revealed the TD was inaccurate: ClockInOut and ScheduleForm already use `useForm` + `zodResolver` + RHF primitives; JobDetailClient uses 1 text input and 2 file uploads (no safeParse). | None — all three components are correct as-is. TD item was stale. | Sprint 7 | Developer | Closed — Inaccurate ✅ |
